...
namespace MiddlewareDemo.middlewares
{
    ...
    public class ServeStaticContentMiddleware
    {
        ...
        private static readonly Dictionary<string, string> _extensionMap = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                { ".323", "text/h323" },
                { ".3g2", "video/3gpp2" },
                { ".3gp2", "video/3gpp2" },
    ...
    public ServeStaticContentMiddleware(RequestDelegate next)
        ...
        {
            // if url starts with wwwroot then server the requested static file
            string? urlPath = context.Request.Path.Value;

            string[]? lstSubPath = urlPath?.Split('/');

            if (lstSubPath?[1] != "wwwroot")
            {
                await _next(context);
                return;
            }
            ...
            string serverStaticContentFilePath = $"{env.ContentRootPath}{string.Join("\\", lstSubPath[1..])}";

            HttpResponse response = context.Response;
            string contentType = string.Empty;
            string fileNameWithExtension = lstSubPath[lstSubPath.Length - 1];
            string extension = fileNameWithExtension.Split('.')[1];
            _extensionMap.TryGetValue("." + extension, out contentType);
